...
phonebook_tp: $(SRCS_common) phonebook_opt.c phonebook_opt.h text_align.c
	$(CC) $(CFLAGS_common) $(CFLAGS_opt) \
		-DIMPL="\"phonebook_opt.h\"" -DTP -o $@ \
...
clean:
	$(RM) $(EXEC) *.o perf.* \
	      	calculate orig.txt opt.txt tp.txt output.txt runtime.png align.txt
